...
> 核心邏輯：平滑流量 \right → 攔截錯誤 \right → 自動重試 \right → 監控崩潰 \right → 自動修復。
...
┌─────────────────────────────────────────────────────────┐
│           Agent / OpenClaw          │
│  (發送 X-Priority: P0/P1/P2 \right → 接收 X-Rate-Health) │
└──────────────────────┬──────────────────────────────────┘
            │ 請求流量
            ▼
     ┌────────────────────────┐
...
*  **自動重試**：針對 `500`, `503`, `504` 錯誤實作指數退避重試 (1s \right → 2s \right → 4s)，消除暫時性伺服器波動導致的輸出中斷。
*  **重啟信號**：若重試耗盡，注入 `X-Gateway-Restart-Suggested` 標籤，觸發系統級重啟流程。

### 3. 自動化監控守護 (System Guardian)
*  **全天候監控**：每 30 秒掃描一次鏈路健康度。
*  **自我修復**：
  *  偵測到 Proxy 崩潰 \right → **自動執行 `launchctl kickstart`**。
  *  偵測到邏輯停滯 \right → **自動執行 `openclaw gateway restart`**。
...
`Google 500 錯誤` \right → `Proxy 自動重試 (3次)` \right → `失敗` \right → `注入重啟信號` \right → `Guardian 偵測日誌異常` \right → `自動重啟 Gateway` \right → `服務恢復` \right → `繼續執行任務`。