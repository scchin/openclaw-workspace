...
### 2. Deep Analysis: Corrupted Session $\rightarrow$ Crash
...
6. **Crash**: The allocation exceeds the process heap limit $\rightarrow$ `FATAL ERROR: Ineffective mark-compacts near heap limit Allocation failed - JavaScript heap out of memory` $\rightarrow$ Process terminates.
