...
- **Mechanism**: Lack of backpressure handling. Gateway pushes data faster than the renderer can process $\rightarrow$ memory accumulates in the WebSocket buffer $\rightarrow$ OOM (Out of Memory).
...
- **Mechanism**: Heavy CPU tasks on the Gateway event loop $\rightarrow$ delayed Heartbeat/Ping frames $\rightarrow$ Socket timeout.
...
Both lead to the same result: **UI Freeze $\rightarrow$ Heartbeat Miss $\rightarrow$ Connection Reset $\rightarrow$ Application Crash.**
