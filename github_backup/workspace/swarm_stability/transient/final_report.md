...
3. **Worker Threads**: For heavy computational tasks (e.g., parsing large logs, complex data processing), move logic from the Main process to a **Node.js Worker Thread**.
   - *Pattern*: `Main Process` $\rightarrow$ `Worker Thread` $\rightarrow$ `Main Process` $\rightarrow$ `Renderer`.
...
2. **Throttled IPC**: Implement a throttle/debounce mechanism for Renderer $\rightarrow$ Main communication to prevent "IPC Flooding".
