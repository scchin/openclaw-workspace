...
def [SENSITIVE_TOKEN_HARD_REDACTED]_tracks_pending_state(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("CLAWTEAM_DATA_DIR", str(tmp_path))
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_backoff(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWTEAM_DATA_DIR", str(tmp_path))
    policy = DefaultRoutingPolicy(team_name="demo", throttle_seconds=30)
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_exec_behavior():
    class DummyMailbox:
...
def [SENSITIVE_TOKEN_HARD_REDACTED](capsys):
    class DummyMailbox:
        def receive(self, *_args, **_kwargs):
            return []
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_on_stdout(capsys):
    class DummyMailbox:
        def receive(self, *_args, **_kwargs):
            return []
