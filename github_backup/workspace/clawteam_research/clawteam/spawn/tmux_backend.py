...
class TmuxBackend(SpawnBackend):
    ...
    def spawn(
        ...
        if is_openclaw_command(command):
            ...
            [SENSITIVE_TOKEN_HARD_REDACTED](env_vars)
        ...
        if skip_permissions:
            if is_claude_command(normalized_command) or is_qwen_command(normalized_command):
                ...
                final_command.append("[SENSITIVE_TOKEN_HARD_REDACTED]")
...
def _startup_prompt_action(command: list[str], pane_text: str) -> str | None:
    ...
    if [SENSITIVE_TOKEN_HARD_REDACTED](command, pane_text):
        return "down-enter"
    if [SENSITIVE_TOKEN_HARD_REDACTED](command, pane_text):
        return "enter"
    return None
...
def [SENSITIVE_TOKEN_HARD_REDACTED](command: list[str], pane_text: str) -> bool:
