...
## 1. 系統架構拓撲
OpenClaw 的基礎設施採用「多層代理 $\rightarrow$ 核心網關 $\rightarrow$ 操作系統」的遞進式設計，旨在將 AI 的邏輯執行與物理環境的存取、安全性、以及輸出清洗完全解耦。

**數據流路徑：**
`LAN 請求 (HTTPS)` $\rightarrow$ `LAN HTTPS Proxy (TLS 終端)` $\rightarrow$ `Injection Proxy (認證注入/內容清洗)` $\rightarrow$ `OpenClaw Gateway (核心網關)` $\rightarrow$ `Runtime/OS (工具執行)`
...
- **金鑰同步鏈**: 強制同步 `.env` $\rightarrow$ `models.json` $\rightarrow$ `auth-profiles.json`，確保 API Key 在三層架構中絕對一致。
...
- **物理級清洗**: 在 Response 返回客戶端前，遞迴掃描 HTML/JSON，將所有 $\text{LaTeX}$ 符號物理替換為 Unicode 符號 (如 `\rightarrow` $\rightarrow$ `→`)，實現零污染輸出。

### 🌐 區網接入優化 (LAN Connectivity)
`lan_https_proxy.py` 解決了遠端存取的性能與安全問題：
- **TLS 終端**: 提供 HTTPS 加密，解決瀏覽器對混合內容 (Mixed Content) 的攔截。
- **Keep-Alive 複用**: 針對 LAN 延遲，採用 TLS 連線保持 $\rightarrow$ 臨時 TCP 請求的模式，大幅降低 TLS 握手開銷。
