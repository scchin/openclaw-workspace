...
### 核心結論
- **協同矩陣 (The Trinity)**：
    - `mempalace` $\rightarrow$ 解決 **上下文/記憶** 的連續性。
    - `system-task-manager` $\rightarrow$ 解決 **物理狀態/進程** 的持久化（跨 Session 追蹤）。
    - `discipline-guardian` $\rightarrow$ 解決 **行為準則/回報紀律** 的強制執行。
- **狀態機恢復 (Recovery State-Machine)**：
    - 啟動時強制進入 `STATUS: RECOVERING` $\rightarrow$ 核對 `pending_tasks.json` $\rightarrow$ 補完報告 $\rightarrow$ 切換至 `STATUS: READY`。
...
| **UI 通訊** | `sync_sidecar` (18793) | AI 核心 $\rightarrow$ UI 客戶端的同步橋接 |
