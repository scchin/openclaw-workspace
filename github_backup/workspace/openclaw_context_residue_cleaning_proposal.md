...
### Step 3: 重置 (Reset) → 狀態回溯
- **物理實現**：實作 `Session-Wall` 與 `Checkpoint Manager`。
- **邏輯**：
    - **原子級重置**：偵測到「邏輯崩潰」時，物理刪除所有 Current Session Token → 從最近一個有效的 `Checkpoint` 恢復。
    - **邊界截斷**：切換任務標籤時，強制截斷對話流，僅注入結構化摘要 (`summary.md`)。
    - **需求分頁**：將大文件/長 Log 移至 `/workspace/sessions/<id>/pages/` → 視窗內僅保留 Page ID。

### Step 4: 對齊 (Align) → 記憶校準
- **物理實現**：執行 `Re-alignment Trigger`。
- **邏輯**：
    - **記憶橋接**：清理後插入隱藏指令：`"Note: Noise cleaned. Current focus: [Target]."` → 消除記憶斷層。
    - **狀態快照更新**：強制更新一次 `AGENT_STATUS` → 確保模型知道目前處於哪個正確步驟。
...
### ⚡ 第一階段：快速突破 (Quick-Win) → [ 1-2 週 ]
- **目標**：立即消除 LaTeX 與 Traceback 污染，降低基礎 Token 消耗。
- **動作**：
    1. 部署 `NoiseRegistry` → 實作基於 Regex 的物理替換算子。
    2. 實作 `ContextCompressor` → 對結構化 JSON 數據進行列式壓縮。
    3. 引入「技能按需掛載」 → 解決技能注入導致的上下文膨脹。

### 🛠️ 第二階段：結構升級 (Structural) → [ 3-5 週 ]
- **目標**：實現狀態可控，杜絕跨 Session 錯誤洩漏。
- **動作**：
    1. 構建 `session_state_manager` → 實現任務單元後的 State Snapshot。
    2. 實作 `Atomic Reset` 接口 → 支持從 Checkpoint 精準恢復。
    3. 建立 `Session-Wall` → 實施強制截斷與結構化摘要同步。

### 🚀 第三階段：性能飛躍 (Evolution) → [ 6 週+ ]
- **目標**：實現工業級的「虛擬內存」上下文管理。
- **動作**：
    1. 實作 `Context-Paging` → 將長輸出物理分頁儲存於磁碟。
    2. 部署 `CGC-Engine` → 實施基於 DAG 關係的自動垃圾回收。
    3. 引入 `SAGE` 注意力權重過濾 → 實現真正的主動衛生系統。
