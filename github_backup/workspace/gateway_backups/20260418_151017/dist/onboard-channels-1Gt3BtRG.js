...
import { l as listBundledChannelSetupPlugins } from "./bundled-CGMeVzvo.js";
...
import { c as listChatChannels, d as isChannelVisibleInSetup, n as formatChannelSelectionLine, t as formatChannelPrimerLine } from "./registry-CENZffQG.js";
import { b as resolveAgentWorkspaceDir, x as resolveDefaultAgentId } from "./agent-scope-KFH9bkHi.js";
import { t as isChannelConfigured } from "./channel-configured-BTEJAT4e.js";
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-BbVfCcz-.js";
import { o as normalizeChannelMeta } from "./loader-DYW2PvbF.js";
import { a as getActivePluginRegistryVersion, g as requireActivePluginRegistry } from "./runtime-BB1a2aCy.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-D33sU6YZ.js";
import { B as runSingleChannelSecretStep, F as resolveAccountIdForConfigure, N as promptResolvedAllowFrom, Q as splitSetupEntries } from "./setup-wizard-helpers-C8R_wm_7.js";
...
import { i as listTrustedChannelPluginCatalogEntries, n as loadChannelSetupPluginRegistrySnapshotForChannel, r as listSetupDiscoveryChannelPluginCatalogEntries, t as ensureChannelSetupPluginInstalled } from "./plugin-install-CkWlfCBX.js";
...
function resolveChannelSetupEntries(params) {
	...
	const installedCatalogEntries = installedCatalogEntriesSource.filter((entry) => !installedPluginIds.has(entry.id) && manifestInstalledIds.has(entry.id) && shouldShowChannelInSetup(entry.meta)).map((entry) => ({
		...entry,
	...
	const installableCatalogEntries = installableCatalogEntriesSource.filter((entry) => !installedPluginIds.has(entry.id) && !manifestInstalledIds.has(entry.id) && shouldShowChannelInSetup(entry.meta)).map((entry) => ({
		...entry,
		meta: normalizeChannelMeta({
			id: entry.id,
			meta: entry.meta
