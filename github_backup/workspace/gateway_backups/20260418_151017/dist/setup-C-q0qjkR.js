...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-5pxHCi7V.js";
import "./config-Q9XZc_2I.js";
import { t as WizardCancelledError } from "./prompts-CKJesNrH.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-Cm4w2zCk.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BReCQFuy.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-B6p0k5iH.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-B2yXrXyP.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-COMSH1QX.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-bDdRlsVg.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-BhUcl1IZ.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DGNNS_M_.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-hbB8wOOC.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-hbB8wOOC.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DGNNS_M_.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-QGZ-Vtox.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-CLnWA2Sk.js");
		const { setupChannels } = await import("./onboard-channels-Cfjf3B6_.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-jQy6iV5o.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-ByLUos0e.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-DXKNHCQF.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-DnUHtiY4.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BvAshZoV.js");
