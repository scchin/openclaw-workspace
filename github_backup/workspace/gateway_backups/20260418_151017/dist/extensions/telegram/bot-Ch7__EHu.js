import { _ as wasSentByBot, a as editMessageTelegram, b as withTelegramApiErrorLogging, g as recordSentMessage, v as buildInlineKeyboard } from "./send-DlzbQJQs.js";
import { l as resolveTelegramMediaRuntimeOptions, s as resolveTelegramAccount } from "./accounts-CoskdHdZ.js";
import { t as normalizeTelegramReplyToMessageId } from "./outbound-params-DPqRaEDE.js";
import { r as resolveTelegramTransport } from "./fetch-B2K4ajFP.js";
import { r as resolveTelegramInlineButtonsScope } from "./inline-buttons-zDRhkQx3.js";
import { f as shouldSuppressLocalTelegramExecApprovalPrompt, l as shouldEnableTelegramExecApprovalButtons, n as isTelegramExecApprovalApprover, r as isTelegramExecApprovalAuthorizedSender } from "./exec-approvals-GJ9TaLpO.js";
import { A as withResolvedTelegramForumFlag, B as resolveTelegramPrimaryMedia, C as resolveTelegramDirectPeerId, E as resolveTelegramGroupAllowFromContext, F as getTelegramTextParts, G as resolveSenderAllowMatch, H as isSenderAllowed, I as hasBotMention, M as buildSenderName, N as expandTextLinks, O as resolveTelegramStreamMode, P as extractTelegramLocation, R as normalizeForwardedContext, S as extractTelegramForumFlag, T as resolveTelegramForumThreadId, U as normalizeAllowFrom, V as firstDefined, W as normalizeDmAllowFromWithStore, _ as buildTelegramParentPeer, b as buildTypingThreadParams, c as isSafeToRetrySendError, g as buildTelegramGroupPeerId, h as buildTelegramGroupFrom, i as renderTelegramHtmlText, j as buildSenderLabel, k as resolveTelegramThreadSpec, l as isTelegramClientRejection, m as buildGroupLabel, p as tagTelegramNetworkError, s as isRecoverableTelegramNetworkError, v as buildTelegramRoutingTarget, w as resolveTelegramForumFlag, x as describeReplyTarget, y as buildTelegramThreadParams } from "./format-DkmJkZf4.js";
import { c as buildModelsKeyboard, d as getModelsPageSize, f as parseModelCallbackData, l as buildProviderKeyboard, p as resolveModelSelection, t as buildCommandsPaginationKeyboard, u as calculateTotalPages } from "./command-ui-Cfr5pQwj.js";
import { i as resolveTelegramCustomCommands, r as normalizeTelegramCommandName, t as TELEGRAM_COMMAND_NAME_PATTERN } from "./command-config-Bv3LfGfn.js";
import { t as createTelegramThreadBindingManager } from "./thread-bindings-ComZmmvs.js";
import { n as cacheSticker, t as describeStickerImage } from "./sticker-cache-CxGsdXlh.js";
import { n as evaluateTelegramGroupPolicyAccess, t as evaluateTelegramGroupBaseAccess } from "./group-access-CWAdf-x0.js";
import { n as deliverReplies, r as emitInternalMessageSentHook, t as resolveMedia } from "./delivery-AYrG1NE_.js";
...
import { DEFAULT_ACCOUNT_ID, buildAgentMainSessionKey, buildAgentSessionKey, deriveLastRoutePolicy, normalizeAccountId, resolveAgentIdFromSessionKey, resolveAgentRoute, resolveThreadSessionKeys, sanitizeAgentId } from "openclaw/plugin-sdk/routing";
...
import { buildPluginBindingResolvedText, getSessionBindingService, isPluginOwnedSessionBindingRecord, parsePluginBindingApprovalCustomId, readChannelAllowFromStore, resolveConfiguredBindingRoute, resolvePluginConversationBindingApproval, resolveThreadBindingIdleTimeoutMsForChannel, resolveThreadBindingMaxAgeMsForChannel, resolveThreadBindingSpawnPolicy, upsertChannelPairingRequest } from "openclaw/plugin-sdk/conversation-runtime";
...
import { DEFAULT_EMOJIS, logAckFailure, logTypingFailure, removeAckReactionAfterReply, resolveAckReaction, shouldAckReaction } from "openclaw/plugin-sdk/channel-feedback";
...
async function loadTelegramSendRuntime() {
	telegramSendRuntimePromise ??= import("./send-DlzbQJQs.js").then((n) => n.p);
...
function resolveTelegramConversationBaseSessionKey(params) {
	if (!(params.route.accountId !== DEFAULT_ACCOUNT_ID && params.route.matchedBy === "default") || params.isGroup) return params.route.sessionKey;
...
async function loadTelegramNativeCommandDeliveryRuntime() {
	telegramNativeCommandDeliveryRuntimePromise ??= import("./bot-native-commands.delivery.runtime-TxLYs6tp.js");
...
async function loadTelegramNativeCommandRuntime() {
	telegramNativeCommandRuntimePromise ??= import("./bot-native-commands.runtime-CcA6vTMr.js");
...
const registerTelegramHandlers = ({ cfg, accountId, bot, opts, telegramTransport, runtime, mediaMaxBytes, telegramCfg, allowFrom, groupAllowFrom, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, processMessage, logger, telegramDeps = defaultTelegramBotDeps }) => {
	...
	bot.on("callback_query", async (ctx) => {
		...
		try {
			...
			if (err instanceof TelegramRetryableCallbackError) throw err.cause;
		}
	});
...
async function resolveStickerVisionSupport$1(params) {
	try {
		const { resolveStickerVisionSupportRuntime } = await import("./sticker-vision.runtime-D9lkiKfU.js");
...
async function resolveTelegramInboundBody(params) {
	...
	if (hasAudio && !hasUserText && (!isGroup || requireMention && mentionRegexes.length > 0 && !disableAudioPreflight && senderAllowedForAudioPreflight)) try {
		const { transcribeFirstAudio } = await import("./media-understanding.runtime-CwHEDu-4.js");
...
async function loadTelegramMessageContextSessionRuntime(runtime) {
	...
	return {
		...await import("./bot-message-context.session.runtime-BHDxyyig.js"),
		...runtime
	};
}
...
async function loadTelegramMessageContextRuntime() {
	telegramMessageContextRuntimePromise ??= import("./bot-message-context.runtime-B2IRS6zW.js");
...
const dispatchTelegramMessage = async ({ context, bot, cfg, runtime, replyToMode, streamMode, textLimit, telegramCfg, telegramDeps = defaultTelegramBotDeps, opts }) => {
	...
	if (statusReactionController && !hasFinalResponse) Promise.resolve(statusReactionController.setError()).catch((err) => {
	...
	if (statusReactionController) Promise.resolve(statusReactionController.setDone()).catch((err) => {
