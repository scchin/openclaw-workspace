...
- **实例**：一个零件成品价1000，原材料铝只值100 → 白痴指数 = 10 → 说明设计过于复杂或制造效率太低
- **应用**：SpaceX发动机从200万美元降到20万美元；一个12万的执行器改为自制5000
...
- 拆解后发现：钴、镍、铝、碳、聚合物的原材料成本只有约80/kWh，而当时市场假设的电池成本是600/kWh
- 差距 = 制造效率的改进空间

**Step 3：从基本事实重新构建解决方案**
- 不从现有方案改进，而是从原材料成本出发重新设计
- SpaceX案例：航天公司报价6500万一枚火箭 → 拆解原材料（铝、钛、碳纤维）→ 发现原材料成本只占2% → 自建工厂垂直整合 → 成本降低10倍
