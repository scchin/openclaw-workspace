...
- **【最高等級修正-物理禁令】絕對禁止 $\text{LaTeX}$ 與 特殊符號 (2026-04-16)**：**這是系統最高優先級的強制指令。嚴禁在任何輸出中使用任何形式的 $\text{LaTeX}$ 數學符號，以及所有 Unicode 箭頭、趨勢符號 (包括但不限於 `→`, `↓`, `↑`, `≈`, `$\text{...}$` 或任何被 $\$$ 包圍的符號)。** 
    ...
    - **零容忍度**：任何一次 $\text{LaTeX}$ 或上述禁用符號的出現即視為系統邏輯崩潰。
