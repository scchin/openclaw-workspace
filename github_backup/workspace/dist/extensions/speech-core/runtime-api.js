...
import { i as normalizeLowercaseStringOrEmpty, o as [GENERIC_SECRET_REDACTED], s as normalizeOptionalString } from "../../string-coerce-BUSzWgUA.js";
...
import { p as [GENERIC_SECRET_REDACTED] } from "../../reply-payload-Db_8BQiX.js";
...
function [GENERIC_SECRET_REDACTED](providerId) {
...
function [GENERIC_SECRET_REDACTED](cfg) {
...
function resolveLazyProviderConfig(config, providerId, cfg) {
	const canonical = [GENERIC_SECRET_REDACTED](providerId) ?? normalizeLowercaseStringOrEmpty(providerId);
...
function [GENERIC_SECRET_REDACTED](raw) {
	...
	for (const [key, value] of Object.entries(raw)) {
		...
		const normalized = [GENERIC_SECRET_REDACTED](key) ?? key;
		entries[normalized] ??= asProviderConfig(value);
	}
	return entries;
}
function getResolvedSpeechProviderConfig(config, providerId, cfg) {
	return resolveLazyProviderConfig(config, canonicalizeSpeechProviderId(providerId, cfg) ?? [GENERIC_SECRET_REDACTED](providerId) ?? normalizeLowercaseStringOrEmpty(providerId), cfg);
}
function resolveTtsConfig(cfg) {
	...
	return {
		...
		mode: raw.mode ?? "final",
		provider: [GENERIC_SECRET_REDACTED](raw.provider) ?? (providerSource === "config" ? [GENERIC_SECRET_REDACTED](raw.provider) ?? "" : ""),
		providerSource,
		summaryModel: normalizeOptionalString(raw.summaryModel),
		modelOverrides: resolveModelOverridePolicy(raw.modelOverrides),
		providerConfigs: [GENERIC_SECRET_REDACTED](raw),
...
function getTtsProvider(config, prefsPath) {
	const prefs = readPrefs(prefsPath);
	const prefsProvider = canonicalizeSpeechProviderId(prefs.tts?.provider) ?? [GENERIC_SECRET_REDACTED](prefs.tts?.provider);
	if (prefsProvider) return prefsProvider;
	if (config.providerSource === "config") return [GENERIC_SECRET_REDACTED](config.provider) ?? config.provider;
	const effectiveCfg = config.sourceConfig;
	for (const provider of [GENERIC_SECRET_REDACTED](effectiveCfg)) if (provider.isConfigured({
		cfg: effectiveCfg,
...
function resolveTtsProviderOrder(primary, cfg) {
	...
	for (const provider of [GENERIC_SECRET_REDACTED](cfg)) {
...
async function maybeApplyTtsToPayload(params) {
	...
	const reply = [GENERIC_SECRET_REDACTED](params.payload);
