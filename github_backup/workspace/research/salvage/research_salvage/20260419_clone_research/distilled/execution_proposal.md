...
| **Boot Test** | Hold Power/Option → Select `Clone_Target` → Boots to Login | Forbidden sign (circle-slash) or Kernel Panic |
...
- **Solution**: Reboot into **Recovery Mode** → Open **Terminal** → Re-run the `asr` command. This bypasses any OS-level locks on the source volume.

### Scenario B: Disk is cloned but won't boot (Forbidden Sign)
- **Cause**: Secure Boot policy on Apple Silicon.
- **Solution**: 
  1. Boot into Recovery.
  2. Use `Startup Security Utility` → Set to **"Reduced Security"** (Allow user management of kernel extensions).
