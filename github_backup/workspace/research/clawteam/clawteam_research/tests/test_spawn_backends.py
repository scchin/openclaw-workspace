...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch, tmp_path):
    ...
    assert cmd[7:] == ["--", "codex", "[SENSITIVE_TOKEN_HARD_REDACTED]", "do work"]


def [SENSITIVE_TOKEN_HARD_REDACTED]_command_and_registry(
    ...
    assert captured["cmd"][7:] == ["--", "codex", "[SENSITIVE_TOKEN_HARD_REDACTED]", "do work"]
    assert registered == {
        "team_name": "demo-team",
        "agent_name": "worker1",
        "backend": "subprocess",
        "pid": 9876,
        "command": ["codex", "[SENSITIVE_TOKEN_HARD_REDACTED]", "do work"],
    }
...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "clawteam.spawn.tmux_backend.[SENSITIVE_TOKEN_HARD_REDACTED]",
        lambda *_, **__: False,
    )
    monkeypatch.setattr(
        "clawteam.spawn.tmux_backend.[SENSITIVE_TOKEN_HARD_REDACTED]",
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_prompt(monkeypatch, tmp_path):
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_missing(monkeypatch):
...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch):
    ...
    confirmed = [SENSITIVE_TOKEN_HARD_REDACTED]("demo:agent", ["claude"])
...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch):
    ...
    confirmed = [SENSITIVE_TOKEN_HARD_REDACTED]("demo:agent", ["codex"])
...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch):
    ...
    dismissed = [SENSITIVE_TOKEN_HARD_REDACTED](
        "demo:agent",
        ["codex"],
        timeout_seconds=2.0,
...
def [SENSITIVE_TOKEN_HARD_REDACTED](monkeypatch):
    ...
    confirmed = [SENSITIVE_TOKEN_HARD_REDACTED]("demo:agent", ["gemini"])
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_local_file_exists(
    monkeypatch, tmp_path
):
...
def [SENSITIVE_TOKEN_HARD_REDACTED]_explicit_directory(
    monkeypatch, tmp_path
):
...
class TestWaitForCliReady:
    ...
    def [SENSITIVE_TOKEN_HARD_REDACTED](self, monkeypatch):
...
def [SENSITIVE_TOKEN_HARD_REDACTED](tmp_path, monkeypatch):
    """Token from openclaw.json should be set in env vars."""
    import json

    from clawteam.spawn.cli_env import [SENSITIVE_TOKEN_HARD_REDACTED]
    ...
    env = {}
    [SENSITIVE_TOKEN_HARD_REDACTED](env)
