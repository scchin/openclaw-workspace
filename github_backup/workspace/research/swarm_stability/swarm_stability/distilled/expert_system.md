...
### The Failure Chain
`Corrupted JSONL (Missing Newline/Quote)` → `Parser consumes multiple lines as one giant string` → `Synchronous JSON.parse() blocks the Event Loop` → `Heap Memory Spike` → `V8 OOM Crash`.
...
- **Atomic Saves**: Implement `write` → `temp file` → `rename` to eliminate "torn writes".
