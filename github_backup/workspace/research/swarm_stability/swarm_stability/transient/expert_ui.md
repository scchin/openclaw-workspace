...
3. **The Loop**: 
   - Renderer is waiting for the response → Renderer's main thread is occupied/blocked.
   - Main process is busy → Cannot send the response.
   - Because the Renderer is blocked, it cannot process "ping" messages or OS-level window updates → The OS detects the window is unresponsive → "Application Not Responding" (ANR) / Window Freeze.
