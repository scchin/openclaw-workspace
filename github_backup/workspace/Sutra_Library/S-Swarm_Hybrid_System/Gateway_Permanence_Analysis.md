...
- **觸發原因**：每當 OpenClaw 進行版本更新、依賴庫更新或執行 `npm install` 等操作時，系統會重新從原始碼編譯或下載最新的 Bundle 檔案 $\rightarrow$ **原本注入的 `_purify` 函數被物理覆蓋** $\rightarrow$ 攔截層瞬間消失。
...
- **執行路徑**：
  `啟動 Gateway` $\rightarrow$ `觸發 Guardian 腳本` $\rightarrow$ `掃描 server.impl-*.js` $\rightarrow$ `檢查是否包含 _purify 簽名` $\rightarrow$ `若缺失 $\rightarrow$ 立即物理注入` $\rightarrow$ `啟動服務`。
...
- **運作層級**：在 LLM 產生答案後 $\rightarrow$ 交付給 Gateway 之前。
- **缺陷**：它依賴於 Agent 的「自覺」去調用。如果 Agent 在某次對話中跳過了 Hook，$\text{LaTeX}$ 就會洩漏。

而 `Gateway 攔截` 屬於 **「傳輸 Hook (Transport Hook)」**：
- **運作層級**：在數據離開伺服器 $\rightarrow$ 進入網路傳輸之前的最後一環。
...
1. **第一層：強制認知 Hook (Agent-side)**
   - 將 `output_sanitizer.py` 定義為所有 Agent 的**強制輸出插件** $\rightarrow$ 確保大部分內容在生成階段即純淨。
2. **第二層：永久傳輸 Hook (Gateway-side)**
   - 實作上述的 `Gateway-Guardian` 啟動注入機制 $\rightarrow$ 確保任何意外洩漏的 $\text{LaTeX}$ 在傳輸前被物理抹除。

**總結**：透過 **「Agent 端的自律 $\rightarrow$ Gateway 端的強權」**，我們能將攔截機制從「依賴外部方式」轉化為系統內生的物理特性。
