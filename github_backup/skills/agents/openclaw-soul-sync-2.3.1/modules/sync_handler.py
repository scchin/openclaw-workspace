...
class SyncHandler:
    ...
    def push_to_github(self, local_path):
        ...
        try:
            ...
            # [CRITICAL] 強制清理本地工作區，消除未提交變更導致的 rebase 失敗
            try:
                print("🧹 [Pulse Push] Cleaning local workspace (reset --hard)...", flush=True)
                self.run_git(["reset", "--hard", "HEAD"], cwd=base_dir)
            except Exception as e:
                print(f"⚠️ [Pulse Push] Workspace reset failed (non-fatal): {e}", flush=True)
