...
echo "⚕️ [Fortress Boot] Running Key Healer Liveness Check..."
python3 "$LOGIC_DIR/key_healer.py"
...
exec /opt/homebrew/opt/node/bin/node \
    --import "$PATCH_PATH" \
    /opt/homebrew/lib/node_modules/openclaw/dist/index.js gateway --allow-unconfigured
