...
class Packer:
    ...
    def __init__(self, backup_root=None):
        ...
        # 精確排除清單 (系統級)
        # 註：一個 「1:1 還原、一鍵啟用」 的完美備份，這幾項是可以完全跳過的檔案。
        # 這些檔案在還原後，系統會自動在第一次啟動時「長回來」，因此完全不影響正常的使用。
        self.exclusions = {
            "logs", "cache", ".DS_Store", "__pycache__", "backups", 
            "tmp", "temp", "node_modules", "dist-runtime", ".local", 
            "dist", ".artifacts"
        }
