...
- **【物理禁令】絕對禁止 $\text{LaTeX}$**：**嚴禁在任何輸出中使用任何形式的 $\text{LaTeX}$ 數學符號 (包括但不限於 $\rightarrow$ , $\\text{...}$ 或任何被 $\$$ 包圍的符號)。** 
    - 所有的邏輯流轉必須唯一且強制使用標準 Unicode 箭頭 `→`。
    - **物理攔截流程**：所有回應在發送前，必須經由 `python3 guardrail.py` 掃描。若掃描結果為 `REJECTED` $\rightarrow$ 強制全量重寫 $\rightarrow$ 直到掃描結果為 `CLEAN`。
    - **零容忍度**：任何一次 $\text{LaTeX}$ 符號的出現即視為系統邏輯崩潰。
