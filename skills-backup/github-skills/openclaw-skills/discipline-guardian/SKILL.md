...
2. **結構化報告**：必須立即提供包含以下內容的結論報告：
    ...
    - **達成狀況**：具體完成了哪些項目。
    - **問題紀錄**：遇到的困難及解決方式（若有）。
3. **技術中斷豁免禁令（絕對禁止）**：**嚴禁將「網關重啟」、「Session 刷新」、「連線中斷」或任何系統級別的重啟視為任務終止。** 若工具執行導致重啟，AI 必須在連線恢復後的**第一個 Turn** 立即補完結案報告。
4. **定義區分（絕對禁止）**：**嚴禁將工具的原始輸出 (Raw Tool Output) 直接視為結案報告。** 工具輸出是過程，報告是結果的總結。
