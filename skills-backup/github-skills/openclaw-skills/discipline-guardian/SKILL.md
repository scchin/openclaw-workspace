...
**觸發條件**：只要 AI 產生了任何行為（Action），即視為一次「工作」。包含但不限於：
- **使用者提問**：所有需要分析、檢索並回答的問題。
- **使用者要求**：所有指令性的操作（如：備份、修改、創建、移動）。
- **技能調用**：任何觸發 `SKILL.md` 邏輯的行為（無論結果是成功、失敗或無輸出）。
- **系統維護**：所有針對配置、文件、環境的任何修改（如：更新紀律、修復路徑）。
- **背景任務**：所有非同步執行的進程一旦結束。
...
3. **強制紀錄機制 (Task Ledger)**：
    - **啟動寫入**：在宣告啟動工作的瞬間，必須同步將任務名稱、啟動時間寫入 `~/.openclaw/workspace/pending_tasks.json`。
    - **結束清除**：僅在提交結案報告後，方可將該項從 `pending_tasks.json` 中移除。
    - **恢復核對**：任何連線恢復、網關重啟或 Session 啟動後的**第一個動作**，必須讀取此檔案並補完所有 `Pending` 狀態的報告。
4. **技術中斷豁免禁令（絕對禁止）**：**嚴禁將「網關重啟」、「Session 刷新」、「連線中斷」或任何系統級別的重啟視為任務終止。** 若工具執行導致重啟，AI 必須在連線恢復後的**第一個 Turn** 立即補完結案報告。
5. **定義區分（絕對禁止）**：**嚴禁將工具的原始輸出 (Raw Tool Output) 直接視為結案報告。** 工具輸出是過程，報告是結果的總結。
