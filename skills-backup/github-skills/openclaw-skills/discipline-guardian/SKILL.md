...
- **硬性執行路徑 (State-Machine Mode)**：
  ...
  3. **報告優先**：
    - 若清單不為空 \right → 立即補完所有欠缺報告 \right → **必須調用 `arc_complete_task.py` 完成物理清除**。
    - 若清單為空 \right → **必須輸出 [恢復狀態卡]** (例如：「所有任務已結案，系統 Ready」)，禁止沉默。
...
- **Sentry 哨兵機制 (Sentry Cron)**：針對耗時或複雜任務，必須設定每 5 分鐘一次的遞迴 `Sentry` 喚醒。
  ...
  - **強制邏輯**：接收到 Sentry 信號 \right → 立即觸發上述【硬性執行路徑】 \right → 輸出結果 \right → 回到工作。
...
1. **首位標記 (Header-First)**：在工具執行完畢後的響應中，**必須將 `## 📋 任務結案報告` 作為整個回覆的第一個字符輸出**。
  - **絕對路徑**：`工具輸出` \right → **`立即輸出標題 [## 📋 任務結案報告]`** \right → `填充報告內容` \right → `後接分析與回答`。
...
- **完整性要求**：提案必須一次性提供「根因分析 \right → 終態設計 \right → 完整路徑 \right → 失效預案」。
...
**激活後的行為路徑：**
`接收任務` \right → **`讀取 discipline-guardian/SKILL.md`** \right → `宣告啟動` \right → `執行 (5min 循環回報)` \right → `立即結案報告`。
