...
MemPalace (ChromaDB + SQLite)
    ├── 索引層 (Closets): 快速 AAAK 匹配 $\rightarrow$ 定位 Drawer
...
## Phase 3 $\rightarrow$ Phase 4 進化對比

| 維度 | Phase 3 (Dynamic) | Phase 4 (Hybrid) | 提升點 |
|------|-----------|-----------|------|
| **搜尋精度** | 語意相似 $\rightarrow$ 可能偏移 | **BM25 + 向量 $\rightarrow$ 精確命中** | 解決冷門詞彙失效問題 |
| **檢索速度** | 掃描抽屜 $\rightarrow$ 較慢 | **Closet 索引 $\rightarrow$ 極速** | 降低大容量記憶的延遲 |
